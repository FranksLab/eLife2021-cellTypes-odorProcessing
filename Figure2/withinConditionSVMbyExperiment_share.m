...
for k = 1:length(KWIKfiles)
    ...
    for lset = 1:length(LR_idx)
        ...
        for iter = 1:numIter
            ...
            randset = randperm(size(data{lset}{k},2),numPerm{k});
            tempdata = data{lset}{k}(:,randset);
...
%                 classes = unique(trl,'stable'); % maintain indexing when points are removed
            ...
            CMat{lset}{k}{iter} = bsxfun(@rdivide,CM{lset},sum(CM{lset},2));
            ACC{lset}{k}(iter) = mean(diag(CMat{lset}{k}{iter}));
        end
    end
end
